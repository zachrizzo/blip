...
export function buildPaperclipEnv(agent: { id: string; companyId: string; budgetMonthlyCents?: number }): Record<string, string> {
  ...
  if (agent.budgetMonthlyCents != null && agent.budgetMonthlyCents > 0) {
    vars.PAPERCLIP_BUDGET_MONTHLY_CENTS = String(agent.budgetMonthlyCents);
  }
  return vars;
}
